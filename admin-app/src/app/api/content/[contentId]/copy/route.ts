...
export async function POST(req:NextRequest,props:{params:Promise<{contentId:string}>}){
  ...
  const item=await db.selectFrom('content_items').select(['id','status','purpose']).where('id','=',contentId).executeTakeFirst();if(!item)return notFound();if(!['draft','ready','failed'].includes(item.status))return badRequest('not_editable');
