...
/** Upload base64 image bytes to the campaign's storage folder → {path, publicUrl}. */
async function uploadImageBytes(db: any, campaignId: string, base64: string, mime: string): Promise<{ path: string; publicUrl: string }> {
  const ext = mime.includes('png') ? 'png' : mime.includes('webp') ? 'webp' : 'jpg';
  const path = `campaigns/${campaignId}/edited-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${ext}`;
  const bytes = Buffer.from(base64, 'base64');
  const { error } = await db.storage.from(storageBucket()).upload(path, bytes, { contentType: mime, upsert: true });
  if (error) throw new Error(error.message);
  const { data: pub } = db.storage.from(storageBucket()).getPublicUrl(path);
  ...
  try {
    switch (action) {
      case 'update': {
        ...
        if (Object.keys(upd).length) await db.from('campaigns').update(upd).eq('id', id);
      ...
      case 'generate_caption': {
        ...
        const { data: cps } = await db
          ...
          .select('product_id, products(product_code, libyan_display_name, arabic_name, english_name, source_name, active_price, category, arabic_keywords)')
          .eq('campaign_id', id)
          .limit(20);
        ...
        await db.from('campaigns').update({ generated_caption: result.text }).eq('id', id);
        ...
        await db.from('activity_logs').insert({ actor_type: 'ai', action: 'campaign_generated', entity_type: 'campaign', entity_id: id, summary: 'Caption generated' });
      ...
      case 'generate_design_prompt': {
        ...
        const { data: cps } = await db
          .from('campaign_products')
          .select('product_id, products(product_code, libyan_display_name, arabic_name, english_name, source_name, category, arabic_keywords)')
          .eq('campaign_id', id)
          .limit(20);
        const products = (cps ?? []).map((cp: any) => ({
          id: cp.product_id,
        ...
        if (!result.ok) return NextResponse.json({ error: 'integration_not_configured', missing: result.missing }, { status: 503 });
        await db.from('campaigns').update({ design_prompt: result.text }).eq('id', id);
        await db.from('ai_events').insert({
          kind: 'image_edit',
          related_id: id,
          model: result.model,
        ...
        await db.from('activity_logs').insert({ actor_type: 'ai', action: 'campaign_generated', entity_type: 'campaign', entity_id: id, summary: 'Design prompt generated' });
      ...
      case 'attach_products': {
        ...
        await db.from('campaign_products').upsert(
          productIds.map((pid, i) => ({ campaign_id: id, product_id: pid, position: i })),
        ...
        for (const im of imgs ?? []) {
          const cur = primaryByProduct.get((im as any).product_id);
          if (!cur || (im as any).is_primary) primaryByProduct.set((im as any).product_id, (im as any).public_url);
        }
        ...
        const assetRows = [...primaryByProduct.entries()].map(([pid, url]) => ({
          campaign_id: id, product_id: pid, kind: 'original_product_image', public_url: url, position: pos++,
        }));
        // Upsert so re-attaching the same product is idempotent — harmless now,
        // and required once migration 0012's unique index on (campaign_id, product_id, kind) is live.
        if (assetRows.length) await db.from('campaign_assets').upsert(assetRows, { onConflict: 'campaign_id,product_id,kind', ignoreDuplicates: true });
      ...
      case 'generate_edits': {
        ...
        const { data: assetsAll } = await db
          .from('campaign_assets').select('id, public_url, product_id, kind')
          .eq('campaign_id', id).not('public_url', 'is', null);
        ...
        for (const src of sources) {
          ...
          try {
            ...
            const up = await uploadImageBytes(db, id, out.data, out.mimeType);
            const { data: asset, error: insErr } = await db.from('campaign_assets').insert({
            ...
            }).select('id, public_url, source_asset_id').single();
        ...
        await db.from('ai_events').insert({ kind: 'image_edit', related_id: id, success: true, output_summary: `Generated ${created.length} edited image(s) via ${modelLabel}${fbNote}${errors.length ? `; ${errors.length} failed` : ''}` });
        await db.from('activity_logs').insert({ actor_type: 'ai', action: 'campaign_image_edit_generated', entity_type: 'campaign', entity_id: id, summary: `Generated ${created.length} edited image(s) via ${modelLabel}${fbNote}${errors.length ? `; ${errors.length} failed` : ''}` });
      ...
      case 'regenerate_edit': {
        if (!geminiStatus().configured) return NextResponse.json({ error: 'integration_not_configured', missing: ['GEMINI_API_KEY'] }, { status: 503 });
        const { data: a } = await db.from('campaign_assets').select('id, source_asset_id, source_prompt, product_id').eq('id', body.assetId).eq('campaign_id', id).maybeSingle();
        if (!a) return NextResponse.json({ error: 'not_found' }, { status: 404 });
        let srcUrl: string | null = null;
        if ((a as any).source_asset_id) {
          const { data: s } = await db.from('campaign_assets').select('public_url').eq('id', (a as any).source_asset_id).maybeSingle();
          srcUrl = (s as any)?.public_url ?? null;
        }
        ...
        const out = res.images?.[0];
        if (!out) {
          await db.from('ai_events').insert({ kind: 'image_edit', related_id: id, success: false, error: 'Gemini returned no image output during regeneration.' });
          return NextResponse.json({
        ...
        const up = await uploadImageBytes(db, id, out.data, out.mimeType);
        await db.from('campaign_assets').update({ storage_path: up.path, public_url: up.publicUrl, approved: false, source_prompt: String(body.prompt || (a as any).source_prompt || '') }).eq('id', (a as any).id);
      ...
      case 'approve_asset': {
        ...
        await db.from('campaign_assets').update({ approved: true }).eq('id', body.assetId).eq('campaign_id', id);
        return NextResponse.json({ ok: true });
      }

      case 'reject_asset': {
        if (!body.assetId) return NextResponse.json({ error: 'no_asset' }, { status: 400 });
        const { data: a } = await db.from('campaign_assets').select('storage_path').eq('id', body.assetId).eq('campaign_id', id).maybeSingle();
        if ((a as any)?.storage_path) { try { await db.storage.from(storageBucket()).remove([(a as any).storage_path]); } catch { /* best effort */ } }
        await db.from('campaign_assets').delete().eq('id', body.assetId).eq('campaign_id', id);
        return NextResponse.json({ ok: true });
      }
      ...
      case 'prepare_posts': {
        ...
        if (body.scheduledFor) await db.from('campaigns').update({ status: 'scheduled', scheduled_for: body.scheduledFor }).eq('id', id);
        await db.from('activity_logs').insert({ actor_type: 'human', action: 'campaign_generated', entity_type: 'campaign', entity_id: id, summary: `Prepared ${r.created} ${mode} post(s)` });
      ...
      case 'publish_post': {
        ...
        if (r.ok) {
          await db.from('campaigns').update({ status: 'published' }).eq('id', id);
