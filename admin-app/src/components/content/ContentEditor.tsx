...
function CopyStep({ detail, editable, busy, phraseDirty, setPhraseDirty, patch, generate, generateCopy }: any) {
  ...
  return <StepCard eyebrow="الخطوة 3 من 4" title="راجع العبارة والكابشن" intro="العبارة والكابشن باللهجة الليبية وقابلان للتعديل. راجع العبارة قبل إنشاء التصميم النهائي.">
    ...
    <label className="block text-sm font-bold text-fg">العبارة على الصورة <span className="font-normal text-danger">مطلوب مراجعتها</span><textarea key={item.image_text||'empty-phrase'} defaultValue={item.image_text ?? ''} onChange={() => setPhraseDirty(true)} onBlur={(e) => { setPhraseDirty(false); if (e.target.value !== (item.image_text ?? '')) patch({ image_text: e.target.value }); }} rows={2} placeholder="ولّد عبارة ثم عدّلها قبل إنشاء التصميم" className="input mt-2 resize-none py-3 text-lg font-bold" /></label>
    ...
    <label className="mt-5 block text-sm font-bold text-fg">الكابشن<textarea key={item.caption||'empty-caption'} defaultValue={item.caption ?? ''} onBlur={(e) => e.target.value !== (item.caption ?? '') && patch({ caption: e.target.value })} rows={5} placeholder="كابشن عربي مختصر لفيسبوك وإنستغرام" className="input mt-2 resize-y py-3 leading-7" /></label>
    <div className="mt-6 rounded-2xl border border-navy/15 bg-navy/[0.035] p-4"><div className="flex items-start gap-3"><WandSparkles className="mt-1 shrink-0 text-navy" size={22} /><div><h3 className="font-bold text-fg">نتيجة واحدة بأعلى جودة</h3><p className="mt-1 text-sm leading-6 text-muted">Gemini 3 Pro Image بدقة 2K، مع فحص تلقائي للمنتج والنص والسعر والعلامة ومحاولتي تصحيح عند الحاجة.</p></div></div><button disabled={!editable || busy || phraseDirty || !item.image_text?.trim() || !item.image_text_approved} onClick={generate} className="btn-primary mt-4 w-full sm:w-auto">{busy === 'generate' ? <Loader2 className="animate-spin" size={17} /> : <Sparkles size={17} />} إنشاء التصميم</button></div>
  </StepCard>;
}
...
function QualitySummary({ run }: { run: Generation }) {
  ...
  return <div className={`mt-5 rounded-xl border p-4 ${run.quality_status === 'verified' ? 'border-success/25 bg-success/6' : 'border-warning/30 bg-warning/8'}`}><div className="flex items-center gap-2">{run.quality_status === 'verified' ? <CheckCircle2 className="text-success" size={19}/> : <CircleAlert className="text-warning" size={19}/>}<b className="text-sm text-fg">{run.quality_status === 'verified' ? 'اجتاز فحص المنتج والنص والسعر والعلامة' : 'اكتمل مع تحذير يحتاج مراجعتك'}</b></div>{warns.length > 0 && <ul className="mt-2 list-inside list-disc space-y-1 text-xs leading-5 text-muted">{warns.map((w,i) => <li key={i}>{w}</li>)}</ul>}<div className="mt-3 flex flex-wrap gap-3 text-xs text-muted"><span>النموذج: {run.source_model || run.requested_model || 'gemini-3-pro-image'}</span><span>المحاولات: {run.attempt_count}</span>{run.finished_at && <span>الانتهاء: {new Date(run.finished_at).toLocaleString('ar-LY')}</span>}</div></div>;
}
