...
**TypeScript types:** `integrations/db/types.ts` (generated by `npm run db:codegen`) + `integrations/db/rows.ts`
...
| `product_images` | One or more images per product. `public_url` (served by Caddy from `MEDIA_ROOT`), `perceptual_hash` (dHash for fingerprinting), `is_primary`. |
...
| `integration_logs` | Low-level integration events: Meta sends/failures, Gemini errors, database errors. |
| `admin_users` | Historical audit table from the Supabase era; auth now lives in env config (migration 0013 detached the FK). |
...
## What not to manually edit in the database
...
- **`ai_behaviors`** — edit via `/ai-control` in the admin app, not directly in SQL. The app validates the schema.
